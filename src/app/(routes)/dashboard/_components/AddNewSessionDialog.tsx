...
function AddNewSessionDialog({ noLabel = false }) {
  const router = useRouter();

  const [note, setNote] = useState<string>();
  const [loading, setLoading] = useState<boolean>(false);
  const [suggestedDoctors, setSuggestedDoctors] = useState<AiAgentDoctor[]>([]);
  const [selectedDoctor, setSelectedDoctor] = useState<AiAgentDoctor | null>(
    null
  );
  ...
  return (
    <>
      ...
      >
        <Phone className={`w-5 ${!noLabel ? "mr-1.5" : ""}`} />
        {!noLabel && "Start Consultation"}
      </button>
      <dialog id="new_session_dialog" className="modal">
        <div className="modal-box">
              ...
              <ul className="list bg-base-100 rounded-box shadow-md">
                {suggestedDoctors.length > 0 &&
                  suggestedDoctors.map((doctor, index) => (
                    <li
                      className={`list-row ${
                        selectedDoctor?.id === doctor.id ? "bg-blue-300" : ""
                      }`}
                      key={index}
                    ...
                    >
                      <div>
                        <Image
                          className="size-10 rounded-box image object-cover"
                          width={120}
                          height={120}
          ...
          <div className="modal-action">
            <form method="dialog">
              <button className="btn" onClick={() => setSuggestedDoctors([])}>
                Close
              </button>
            </form>
            {!suggestedDoctors.length ? (
              ...
              >
                {loading ? (
                  ...
                  <>
                    Start Consultation <ArrowRight className="w-4" />
                  </>
                )}
              </button>
            )}
